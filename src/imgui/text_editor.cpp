...
void TextEditor::HandleKeyboardInputs()
{
	...
	if (ImGui::IsWindowFocused())
	{
		...
		// 	mOverwrite ^= true;
			...
			SelectAll();
		else if (!IsReadOnly() && !ctrl && !alt && /*&& !shift*/ (ImGui::IsKeyPressed(ImGui::GetKeyIndex(ImGuiKey_Enter)) || ImGui::IsKeyPressed(ImGui::GetKeyIndex(ImGuiKey_KeypadEnter))))
			EnterCharacter('\n', false);
			...
			if (!line.empty())
			{
				...
				else
				{
					...
					else
					{
						...
						else if (!withinSingleLineComment && startStr.size() && currentIndex + startStr.size() <= line.size() &&
...
const TextEditor::LanguageDefinition& TextEditor::LanguageDefinition::Bobob() {
	static bool inited = false;
	static LanguageDefinition langDef;
	if (!inited)
	{
		for (auto& k : get_constant_strs())
			langDef.mKeywords.insert(k);
        for (auto& k : get_reg_strs())
            langDef.mKeywords.insert(k);

		for (auto& k : get_op_strs())
		{
			Identifier id;
